...
void TestRecCompute(string codeBookFileName, string word){
	...
	// recording_module 
	time = "4 ";  // no. of seconds for recording
...
{ 
    ...
    //system("clear"); 
    //system("CLS"); 
    cout << setfill(' ') << setw(46) << "      MATRIX GRID      \n"; 
...
void trainableModule(string roll){
	...
	for(int i=0;i<10;i++){
		...
		for(int j=0;j<TRAINABLE_MODULE_REC;j++){
			...
			cout<<f1<<" "<<f2<<"\n";
			string command = "Recording_Module.exe 5 " + foldername+"\\"+f1 + " " + foldername+"\\"+f2;
