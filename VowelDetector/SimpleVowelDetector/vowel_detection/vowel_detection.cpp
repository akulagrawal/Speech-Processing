...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	vector<vector<vector<ld> > > c(nSamples+1, vector<vector<ld> >(nFrames+1, vector<ld>(p+1, 0.0)));
	vector<string> heading;
    
    for(int i=0;i<5;i++){
		for(int j=1;j<=nSamples;j++){
			string fileName = "./Vowels/160101085_" + vowel[i] + "_" + to_string((ll)j) + ".txt";
			c[j] = calculateCFromScratch(fileName, N, p, nFrames, isHeader);
		}
		vector<ld> avg;
		for(int k=0;k<nFrames;k++){
			avg.pb(0.0);
		...
		heading.clear();
		writeVecToFile(avg, "./Vowels/c_" + vowel[i] + ".txt", heading);
	}
	mode = chooseMode();
	if(mode == "1")
		runTests(N,p);
